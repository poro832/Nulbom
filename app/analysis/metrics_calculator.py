...
#
# 목록에 "힘듭", "외롭"처럼 사전적으로는 활용형인 것도 별도 항목으로 들어 있는
# 이유: 불규칙 활용은 어미가 바뀌는 게 아니라 어간 자체의 글자가 바뀐다.
# "힘들"+"습니다" → "힘듭니다"(ㄹ 탈락, "들"이 아예 사라지고 "듭"이 된다),
# "외롭"+"습니다" → "외롭습니다"("외로"가 아니라 애초에 "외롭"이 표면형이다).
# 이런 경우는 어간 뒤에 어미를 아무리 잘 정의해도 못 잡는다 — 찾고 있는
# 문자열 자체가 그 자리에 없기 때문이다. 그래서 형태소 분석기 없이 고칠 수
# 있는 정직한 방법은 그 불규칙 표면형을 사전에 통째로 추가하는 것뿐이다.
DEFAULT_NEGATIVE_STEMS = frozenset(
    {
        "아프", "아파", "아팠", "아픕", "힘들", "힘듭", "외로", "외롭",
        "귀찮", "죽겠", "우울",
    }
)
...
# "아파트"의 "트", "아프리카"의 "리", "아프간"의 "간", "힘듭"/"외롭" 새 어간이
# 노출하는 다른 명사와의 우연한 충돌도 전부 이 화이트리스트로 걸러야 한다 —
# 그리고 실제로 "힘듭"/"외롭"으로 시작하면서 이 어간들과 무관한 일반 명사는
# 조사한 범위에서 찾지 못했다(테스트로 이 판단 자체를 고정해 둔다).
#
...
# (힘들→힘듭, 외로→외롭처럼) 이 규칙이 개입할 기회조차 없다 — 화이트리스트를
# 아무리 넓혀도 없는 문자열은 못 찾는다. 그래서 "애매하면 센다"는 국소 규칙과
# 별개로, 이 시스템의 진짜 위험은 언제나 위음성 쪽이다: 어르신이 "힘듭니다",
# "외롭습니다"라고 말했는데 그 표면형이 아직 사전에 없으면 조용히 놓친다.
# 위양성(멀쩡한데 잡음)은 보호자가 통화를 들어보면 바로 해소되는 성가심이지만,
# 위음성은 이 서비스가 존재하는 이유 자체를 무력화하므로 항상 위음성 쪽을
# 더 두려워해야 한다.
#
# 그래도 이 방법이 놓치는 것 — 정직하게 적어 둔다:
# 1) 아직 사전에 없는 불규칙 표면형이나 활용형. 이번에 9개 어간의 합니다체를
#    전수 조사해서 "힘듭"/"외롭"(어간 자체가 바뀜)과 "우울"+"합"(아래 예외
#    목록, "하"와 다른 음절)을 채웠지만, 존댓말이 아닌 다른 활용(예: 간접
#    인용 "-다더라", 방언, "-으오"체 등)에서 또 다른 불규칙 표면형이 나올
#    수 있고 지금은 다루지 않는다.
# 2) 다음 글자가 한글 음절인데 화이트리스트/어간별 목록에 없는 새로운 활용형이나
#    새 충돌 명사. 이 목록들은 지금 가진 어간들의 실제 활용형을 조사해서
#    채운 것이라 완전하지 않다.
# 3) 어미가 아니라 명사 접미사가 붙어도 진짜 부정 신호인 경우 — 대표적으로
...
# "우울한"도 여기 넣는다. "합"은 "하"+"ㅂ니다"가 합쳐진 별개의 완성
# 음절이라("하다"류 형용사가 전부 이렇게 합니다체를 만든다) 화이트리스트가
# 아니라 여기 넣었다 — "우울합니다"를 잡기 위함이다.
_NOUN_SUFFIX_EXCEPTIONS: dict[str, frozenset[str]] = {
    "우울": frozenset({"증", "하", "해", "했", "한", "합"}),
}
