"""통화에서 결정론적 행동 지표를 계산한다 (설계 3.2).

이 모듈에는 LLM도 네트워크도 개입하지 않는다. 같은 입력이면 반드시 같은
숫자가 나오며, 그래야 "오늘 62점 내일 71점" 같은 일이 생기지 않는다.
LLM은 여기서 나온 숫자를 사람 말로 설명하는 역할만 한다.

**이 숫자가 무엇이 아닌지 먼저 적어 둔다.** 함수 이름이 risk_score라서
임상 점수처럼 읽히기 쉬운데, 아니다.

- 확률이 아니고, 의학적 척도가 아니고, 우울증 진단이 아니다.
- "평소의 이 어르신과 비교해 오늘 대화 행동이 얼마나 달라졌나"를 한 눈금에
  모은 값이다. 제품에서 부르는 이름은 "대화 행동 변화 점수"다.
- 가중치(35/25/20/20)와 임계값(30/60), 절대 기준(0.4 / 2초 / 4초)은
  **실제 통화 데이터를 보기 전 단계의 프로토타입 휴리스틱**이다. 임상적으로
  검증된 값이 아니며, 그렇게 소개해서도 안 된다. 실통 데이터로 각 지표의
  분포와 개인별 변동성을 확인한 뒤 보정할 자리다.

식별자(risk_score, risk_level, assess_risk)는 db/schema.sql의 컬럼명과
맞추려고 그대로 둔다 — 이름을 바꾸면 마이그레이션이 붙는데 얻는 게 없다.
바뀌는 것은 사람에게 보여주는 말이다.
"""
